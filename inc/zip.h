...
#pragma once
#ifndef INCLUDE_ALICORN_CH_ZIP
#define INCLUDE_ALICORN_CH_ZIP
...
#define ZIP_EFWRITE -29     // fwrite error
   ...
   * @return error message string coresponding to errnum or NULL if error is
   * not found.
   */
   ...
   */
  extern ZIP_EXPORT int zip_entry_open(struct zip_t *zip,
   ...
   * contain a drive or device letter, or a leading slash. All slashes MUST be
   * forward slashes '/' as opposed to backwards slashes '\' for compatibility
   ...
   * @return the uncompressed size in bytes.
   */
  extern ZIP_EXPORT unsigned long long
   ...
   * @return the return code - 0 on success, negative number (< 0) on error.
   */
  extern ZIP_EXPORT int zip_entry_fwrite(struct zip_t *zip,
                                         const char *filename);
   ...
   * @return the return code - 0 on success, negative number (< 0) on error.
   */
  extern ZIP_EXPORT int zip_entry_fread(struct zip_t *zip,
                                        const char *filename);
   ...
   * @param zip zip archive handler.
   *
   * @return the return code - the number of entries on success, negative
   * number
   *         (< 0) on error.
   */
   ...
   * Returning a negative value from the callback will cause abort and return
   * an error. The last argument (void *arg) is optional, which you can use to
   * pass data to the on_extract callback.
   *
   ...
   * @return the return code - 0 on success, negative number (< 0) on error.
   */
  extern ZIP_EXPORT int zip_create(const char *zipname,
                                   const char *filenames[], size_t len);
   ...
   * Returning a negative value from the callback will cause abort and return
   * an error. The last argument (void *arg) is optional, which you can use to
   * pass data to the on_extract_entry callback.
   *
   * @param zipname zip archive file.
   * @param dir output directory.
