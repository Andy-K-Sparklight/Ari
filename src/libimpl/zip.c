...
static char *
zip_strrpl(const char *str, size_t n, char oldchar, char newchar)
{
  char c;
  size_t i;
...
static char *
zip_name_normalize(char *name, char *const nname, size_t len)
{
  size_t offn = 0;
...
static mz_bool
zip_name_match(const char *name1, const char *name2)
{
  char *nname2 = NULL;
...
static int
zip_archive_truncate(mz_zip_archive *pzip)
{
...
static int
zip_archive_extract(mz_zip_archive *zip_archive, const char *dir,
    ...
    || defined(__MINGW32__)
          ...
          if(info.m_uncomp_size > MZ_ZIP_MAX_ARCHIVE_FILENAME_SIZE
             || !mz_zip_reader_extract_to_mem_no_alloc(
                 zip_archive, i, symlink_to, MZ_ZIP_MAX_ARCHIVE_FILENAME_SIZE,
                 0, NULL, 0))
            {
              err = ZIP_EMEMNOALLOC;
              goto out;
...
static ssize_t
zip_entry_mark(struct zip_t *zip, struct zip_entry_mark_t *entry_mark,
...
static ssize_t
zip_index_next(mz_uint64 *local_header_ofs_array, ssize_t cur_index)
{
...
static ssize_t
zip_sort(mz_uint64 *local_header_ofs_array, ssize_t cur_index)
{
...
static int
zip_index_update(struct zip_entry_mark_t *entry_mark, ssize_t last_index,
                 ssize_t nxt_index)
{
  ssize_t j;
...
static int
zip_entry_finalize(struct zip_t *zip, struct zip_entry_mark_t *entry_mark,
                   const ssize_t n)
{

  ssize_t i = 0;
  mz_uint64 *local_header_ofs_array
      = (mz_uint64 *)calloc(n, sizeof(mz_uint64));
  ...
  for(i = 0; i < n - 1; i++)
    {
      length[i] = (size_t)(local_header_ofs_array[i + 1]
                           - local_header_ofs_array[i]);
    }
  length[n - 1]
...
static ssize_t
zip_entry_set(struct zip_t *zip, struct zip_entry_mark_t *entry_mark,
              ssize_t n, char *const entries[], const size_t len)
{
  ssize_t err = 0;
...
static ssize_t
zip_file_move(MZ_FILE *m_pFile, const mz_uint64 to, const mz_uint64 from,
              const size_t length, mz_uint8 *move_buf,
...
static ssize_t
zip_files_move(MZ_FILE *m_pFile, mz_uint64 writen_num, mz_uint64 read_num,
               size_t length)
{
  ssize_t n = 0;
...
static int
zip_central_dir_move(mz_zip_internal_state *pState, int begin, int end,
                     int entry_num)
{
  if(begin == entry_num)
    {
  ...
  if(next && l_size == 0)
    {
      ...
      pState->m_central_dir.m_p
          = MZ_REALLOC(pState->m_central_dir.m_p, r_size);
      {
        int i;
...
static int
zip_central_dir_delete(mz_zip_internal_state *pState,
                       int *deleted_entry_index_array, int entry_num)
{
  int i = 0;
  int begin = 0;
  int end = 0;
...
static ssize_t
zip_entries_delete_mark(struct zip_t *zip, struct zip_entry_mark_t *entry_mark,
                        int entry_num)
{
...
struct zip_t *
zip_open(const char *zipname, int level, char mode)
{
...
static int
_zip_entry_open(struct zip_t *zip, const char *entryname, int case_sensitive)
{
  size_t entrylen = 0;
...
int
zip_entry_open(struct zip_t *zip, const char *entryname)
{
  return _zip_entry_open(zip, entryname, 0);
}

int
zip_entry_opencasesensitive(struct zip_t *zip, const char *entryname)
{
  return _zip_entry_open(zip, entryname, 1);
}

int
zip_entry_openbyindex(struct zip_t *zip, size_t index)
{
  mz_zip_archive *pZip = NULL;
  mz_zip_archive_file_stat stats;
  mz_uint namelen;
...
const char *
zip_entry_name(struct zip_t *zip)
{
  if(!zip)
    {
      // zip_t handler is not initialized
...
ssize_t
zip_entry_index(struct zip_t *zip)
{
  if(!zip)
    {
      // zip_t handler is not initialized
...
int
zip_entry_isdir(struct zip_t *zip)
{
  if(!zip)
    {
      // zip_t handler is not initialized
...
unsigned long long
zip_entry_size(struct zip_t *zip)
{
  return zip_entry_uncomp_size(zip);
}

unsigned long long
zip_entry_uncomp_size(struct zip_t *zip)
{
  return zip ? zip->entry.uncomp_size : 0;
}

unsigned long long
zip_entry_comp_size(struct zip_t *zip)
{
  return zip ? zip->entry.comp_size : 0;
}

unsigned int
zip_entry_crc32(struct zip_t *zip)
{
  return zip ? zip->entry.uncomp_crc32 : 0;
}

int
zip_entry_write(struct zip_t *zip, const void *buf, size_t bufsize)
{
  mz_uint level;
  mz_zip_archive *pzip = NULL;
  tdefl_status status;
...
int
zip_entry_fwrite(struct zip_t *zip, const char *filename)
{
  int err = 0;
  size_t n = 0;
  MZ_FILE *stream = NULL;
...
ssize_t
zip_entry_read(struct zip_t *zip, void **buf, size_t *bufsize)
{
  mz_zip_archive *pzip = NULL;
  mz_uint idx;
  size_t size = 0;
...
ssize_t
zip_entry_noallocread(struct zip_t *zip, void *buf, size_t bufsize)
{
  mz_zip_archive *pzip = NULL;
...
int
zip_entry_fread(struct zip_t *zip, const char *filename)
{
  mz_zip_archive *pzip = NULL;
  mz_uint idx;
  mz_uint32 xattr = 0;
...
ssize_t
zip_entries_total(struct zip_t *zip)
{
  if(!zip)
    {
      // zip_t handler is not initialized
...
ssize_t
zip_stream_copy(struct zip_t *zip, void **buf, size_t *bufsize)
{
  size_t n;
...
void
zip_stream_close(struct zip_t *zip)
{
  if(zip)
    {
      mz_zip_writer_end(&(zip->archive));
...
int
zip_create(const char *zipname, const char *filenames[], size_t len)
{
  int err = 0;
  size_t i;
  mz_zip_archive zip_archive;
...
int
zip_extract(const char *zipname, const char *dir,
